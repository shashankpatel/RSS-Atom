		...
		switch (feedType) {
			case FeedTypeRSS: {
				...
				if (!processed) {
					...
					else if ([currentPath isEqualToString:@"/rss/channel/item/dc:creator"]) { if (currentText.length > 0) item.author = [NSString stringWithFormat:@"By %@",currentText]; processed = YES; }
					//NSLog(currentPath);
				}
